...
const checks = [
  ...
  ['evidence groups', () => ['支持当前判断','排除其他原因','当前不能证明'].every(x => html.includes(x))],
  ...
  ['legacy demo title', () => html.includes('既有演示能力')],
  ...
  ['evidence anchor', () => count(/id="task-evidence-quality"/g) === 1 && /id="task-evidence-quality"[^>]*><h2[^>]*>证据与数据质量<\/h2>/.test(html)],
  ['staff quick questions in staff section', () => count(/当前诊断上下文/g) === 1 && /<section v-if="currentView === 'staff'"[\s\S]*?快速了解[\s\S]*?查看依据[\s\S]*?准备处理[\s\S]*?<\/section>/.test(html)],
  ...
  ['structured task branch rendered', () => count(/任务内语境化智能参谋/g) === 1 && /任务内语境化智能参谋[\s\S]{0,4000}?taskAssistantListEl[\s\S]{0,4000}?v-if="m\.structured"[\s\S]{0,4000}?v-model="taskAssistantInput"/.test(html)],
