...
async function run() {
  const old = process.env.STAGING_API_TOKEN; process.env.STAGING_API_TOKEN = 'placeholder-token';
  await test('adapter/auth-env-injection', async()=>{ let seen; const a=new StagingMediaAdapter({fetch:async(u,o)=>{seen=o.headers.Authorization;return {ok:true,status:200,headers:new Map(),json:async()=>({rta_id:'x',source_record_id:'s',updated_at:new Date().toISOString()})}},baseUrl:'https://x.invalid'}); await a.getDataEnvelope({rtaId:'x',timeRange:{start:'2026-01-01',end:'2026-01-01'}}); assert.strictEqual(seen,'Bearer placeholder-token'); delete process.env.STAGING_API_TOKEN; await assert.rejects(()=>a.getDataEnvelope({rtaId:'y',timeRange:{start:'2026-01-01',end:'2026-01-01'}}),e=>e.code==='AUTH_REQUIRED'); process.env.STAGING_API_TOKEN='placeholder-token'; });
